...
sifeats="ark,s,cs:copy-feats scp:$sdata/JOB/feats.scp ark:- |"
featsub_warped="ark,s,cs:copy-feats ark:$dir/feats.CLASS.ark ark:- |"
featsub_unwarped="ark,s,cs:copy-feats ark:$dir/feats.$default_class.ark ark:- |"

if [ "${deltas}" == "true" ]; then
    delta_opts=`cat $fgmm_model/delta_opts 2>/dev/null`
    sifeats="$sifeats add-deltas $delta_opts ark:- ark:- |"
    featsub_warped="$featsub_warped add-deltas $delta_opts ark:- ark:- |"
    featsub_unwarped="$featsub_unwarped add-deltas $delta_opts ark:- ark:- |"
fi

if [ "${cmvn}" == "true" ]; then
    sifeats="$sifeats apply-cmvn-sliding ark:- ark:- |"
    #no cmvn for featsub warped.
fi

if [ "${deltas_sdc}" == "true" ]; then
    sifeats="$sifeats add-deltas-sdc ark:- ark:- |"
    featsub_warped="$featsub_warped add-deltas-sdc $delta_opts ark:- ark:- |"
    featsub_unwarped="$featsub_unwarped add-deltas-sdc $delta_opts ark:- ark:- |"
fi

if [ "${vad}" == "true" ]; then
    if [ ! -f $sdata/vad.scp ]; then echo "No $sdata/JOB/vad.scp. Exiting." && exit 1; fi
    sifeats="$sifeats select-voiced-frames ark:- scp,s,cs:$sdata/JOB/vad.scp ark:- |"
    featsub_warped="$featsub_warped select-voiced-frames ark:- scp,s,cs:$data/JOB/vad.scp ark:- |"
    featsub_unwarped="$featsub_unwarped select-voiced-frames ark:- scp,s,cs:$data/JOB/vad.scp ark:- |"
fi

sifeats="$sifeats subsample-feats --n=$subsample ark:- ark:- |"
featsub_warped="$featsub_warped subsample-feats --n=$subsample ark:- ark:- |"
featsub_unwarped="$featsub_unwarped subsample-feats --n=$subsample ark:- ark:- |"

#------------------------------------    


echo "sifeats = $sifeats"
echo "featsub_warped = $featsub_warped"
echo "featsub_unwarped = $featsub_unwarped"
