...
## MDS: only change is to have a "blind" option for when we don't have the labels for test set. #TODO: see later if actually useful.
...
if [ "$blind" != "true" ] && [ ! -f $data/utt2lang ]; then
    [ ! -f $f ] && echo "No such file $f" && exit 1;
fi
